    ...
    try {
      if (validation.validateSignup(req, res)) {
        ...
        bcrypt.hash(req.body.password, 10, (err, hash) => {
          ...
          const id = user.id; const firstName = user.firstName; const lastName = user.lastName; const email = user.email; const
            type = user.type;
          res.status(201).json({
            status: 201,
    ...
    try {
      ...
      if (validation.validateLogin(req, res)) {
        bcrypt.hash(req.body.password, 10, (err, hash) => {
          ...
          bcrypt.compare(req.body.password, userPassword.password, (err, result) => {
                        if (result == false) {
                            return res.status(401).json({
                                status:401,
                                message: "Incorrect email or password"
